...
export function SynthwaveCanyonShader({
  ...
  useEffect(() => {
    ...
    // Detect mobile to avoid resize on scroll (browser chrome show/hide)
    const isMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent)

    const setSize = () => {
      if (isMobile) {
        // Use screen dimensions — fixed, unaffected by browser address bar
    ...
    // Mobile: only resize on orientation change, not scroll
    // Desktop: resize on window resize
    if (isMobile) {
